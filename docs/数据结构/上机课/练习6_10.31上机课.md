...
> [数据结构——中缀转后缀表达式 - 王陸 - 博客园 (cnblogs.com)](https://www.cnblogs.com/wkfvawl/p/12864789.html)
>
> 从左到右开始扫描中缀表达式
> 遇到数字， 直接输出
> 遇到运算符
...
btNode* suffixTotree(string suffix) {
	stack<btNode*> s;
	for (char c : suffix) {
		if (isalpha(c)) {
			s.push(new btNode(c));
...
void print(btNode* root, int layer) {
	if (root == NULL)
		return;
	print(root->right, layer + 1);
	for (int i = 0; i < layer; i++) {
		cout << '\t';
	}
	cout << root->data << endl;
	print(root->left, layer + 1);
}

int main() {
	string infix;
	cin >> infix;
	print(suffixTotree(infixTosuffix(infix)), 0);
	return 0;
}
...
public:
	...
	int cntLeaves(TreeNode* node) {
		if (node == nullptr)
			return 0;
		if (node->left == nullptr && node->right == nullptr)
			return 1;
		return cntLeaves(node->left) + cntLeaves(node->right);
	}
...
TreeNode* buildTree(vector<int> a,TreeNode* root,int i,int n) {
	if (i < n) {
...
int main() {
	...
	for (int i = 0; i < n; i++)
		a[i] = i;
	nku.root = buildTree(a, nku.root, 0, n);
	cout << "叶节点的数目: " << nku.cntLeaves(nku.root) << endl;
	cout << "层次遍历：";
	nku.traversal(nku.root);
	nku.swapChildren(nku.root);
	cout << "交换左右子树：";
	nku.traversal(nku.root);
	cout << "宽度：" << nku.maxWidth(nku.root) << endl;
	return 0;
}
